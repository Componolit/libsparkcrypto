...
proof -
  ...
  also have "\<dots> =
    ?x ^ nat ((?e * 2 ^ nat (31 - j) +
      \<lfloor>elts e i1\<rfloor>\<^sub>s div 2 ^ nat (j + 1)) * 2 + 1) *
    ?R mod ?m"
  ...
  have "(?e * 2 ^ nat (31 - j) +
      \<lfloor>elts e i1\<rfloor>\<^sub>s div 2 ^ nat (j + 1)) * 2 + 1 =
    ?e * 2 ^ nat (31 - j + 1) +
    \<lfloor>elts e i1\<rfloor>\<^sub>s div 2 ^ nat j div 2 * 2 + 1"
    by (simp only: nat_add_distrib)
      (simp add: zdiv_zmult2_eq [of 2, simplified mult.commute [of _ 2]])
  also from `_ = (if (if elts e i1 AND of_int 2 ^ nat j = of_int 0 then _ else _) \<noteq> _ then _ else _)`
