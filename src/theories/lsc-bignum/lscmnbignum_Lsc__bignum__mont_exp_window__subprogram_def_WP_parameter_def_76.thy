...
proof -
  ...
  have "uint w * (2 ^ nat (j2 - s2 - 1) * 2 ^ 1) =
    ?e div 2 ^ nat (uint i - (j2 - 1)) mod 2 ^ nat j2 * 2"
    by (simp add: num_of_lint_lower)
  also from e have "\<dots> mod Base OR 1 =
    ?e div 2 ^ nat (uint i - (j2 - 1)) mod 2 ^ nat j2 * 2 + 1"
    by (simp add: mod_pos_pos_trivial OR_plus1)
  also from `BV64.ule (of_int j2) i`
    `_ \<longrightarrow> natural_in_range j2`
  have "\<dots> =
    (?e div 2 ^ nat (uint i - j2) div 2 * 2 + 1) mod (2 ^ nat (j2 + 1))"
  ...
  also from i `BV64.ule (of_int j2) i`
    `_ = (elts e (e_first + uint ((i - of_int j2) div of_int 32)) AND
     _ \<noteq> of_int 0)`
    `bit_set e e_first (i - of_int j2) = _`
    uint_lt [of i]
    `_ \<longrightarrow> natural_in_range j2`
    pos_mod_bound [of 32 "uint i - j2"]
  have "?e AND 2 ^ nat (uint i - j2) \<noteq> 0"
  ...
  then have "?e div 2 ^ nat (uint i - j2) div 2 * 2 + 1 =
