theory lscmnbignum_Lsc__bignum__mont_exp_window__subprogram_def_WP_parameter_def_76
imports "../Mont_Mult_Aux"
begin

why3_open "lscmnbignum_Lsc__bignum__mont_exp_window__subprogram_def_WP_parameter_def_76.xml"
...
proof -
  ...
  from `h2 \<le> \<lfloor>r224b\<rfloor>\<^sub>\<nat>` `h1 = h2 + 1` `\<not> h1 \<le> \<lfloor>r224b\<rfloor>\<^sub>\<nat>`
  have "h2 = \<lfloor>r224b\<rfloor>\<^sub>\<nat>" by simp
  ...
  have "num_of_big_int (word32_to_int o a2) \<lfloor>a_first1\<rfloor>\<^sub>\<nat> ?L =
  ...
  also note `(num_of_big_int' (Array aux33 _) _ _ = _) = _`
    [simplified `h2 = \<lfloor>r224b\<rfloor>\<^sub>\<nat>` `\<lfloor>l\<rfloor>\<^sub>\<nat> = \<lfloor>a_last1\<rfloor>\<^sub>\<nat> - \<lfloor>a_first1\<rfloor>\<^sub>\<nat>` base_eq, simplified]
  also {
    ...
    then have "(2::int) ^ nat \<lfloor>r224b\<rfloor>\<^sub>\<nat> \<le> 2 * 2 ^ nat \<lfloor>k\<rfloor>\<^sub>\<nat>"
      ...
      by simp
    with
      `\<forall>n. 0 \<le> n \<and> n \<le> 2 ^ nat \<lfloor>k\<rfloor>\<^sub>\<nat> - 1 \<longrightarrow> (num_of_big_int' (Array aux4 _) _ _ = _) = _`
      `\<lfloor>l\<rfloor>\<^sub>\<nat> = \<lfloor>a_last1\<rfloor>\<^sub>\<nat> - \<lfloor>a_first1\<rfloor>\<^sub>\<nat>`
