...
proof -
  ...
  moreover from `_ = (if \<lfloor>a1 i\<rfloor>\<^bsub>w32\<^esub> < \<lfloor>elts b \<lfloor>o2\<rfloor>\<^sub>\<nat>\<rfloor>\<^bsub>w32\<^esub> then _ else _)`
  have "num_of_bool True = num_of_bool
