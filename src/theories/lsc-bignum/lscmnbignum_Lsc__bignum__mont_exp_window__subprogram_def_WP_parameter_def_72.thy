...
proof -
  ...
  have "num_of_big_int (word32_to_int o a) a_first ?L =
    ...
    by (simp add: power_mult_distrib mult.assoc base_eq map__content_def)
  with Base_inv `(math_int_of_int s \<le> math_int_from_word i + _) = _`
    `BV64.ult i (of_int s)` `_ \<longrightarrow> natural_in_range s`
  show ?thesis by (simp add: BV64.ult_def word_of_int uint_word_of_int
    natural_in_range_def mod_pos_pos_trivial map__content_def)
qed
