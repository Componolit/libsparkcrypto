theory lscmnbignum_Lsc__bignum__mod_add__subprogram_def_WP_parameter_def_2
imports "../LibSPARKcrypto"
begin

why3_open "lscmnbignum_Lsc__bignum__mod_add__subprogram_def_WP_parameter_def_2.xml"

why3_vc WP_parameter_def
proof -
  let ?k = "\<lfloor>a_last1\<rfloor>\<^sub>\<nat> - \<lfloor>a_first1\<rfloor>\<^sub>\<nat> + 1"
  have "num_of_big_int' b \<lfloor>b_first\<rfloor>\<^sub>\<nat> ?k < Base ^ nat ?k"
    by (simp add: num_of_lint_upper word32_to_int_upper')
  moreover have "num_of_big_int' c \<lfloor>c_first\<rfloor>\<^sub>\<nat> ?k < Base ^ nat ?k"
    by (simp add: num_of_lint_upper word32_to_int_upper')
  ultimately have "num_of_big_int (word32_to_int o a) \<lfloor>a_first1\<rfloor>\<^sub>\<nat> ?k <
    ...
    using
      `(num_of_big_int' b _ _ \<le> _) = _ \<or>
       (num_of_big_int' c _ _ \<le> _) = _`
