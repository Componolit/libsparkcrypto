theory lscmnec_Lsc__ec__two_point_mult__subprogram_def_WP_parameter_def_12
imports "../Elliptic_Spec"
begin

why3_open "lscmnec_Lsc__ec__two_point_mult__subprogram_def_WP_parameter_def_12.xml"
...
proof (simp add: two_point_mult_spec_def Let_def, (rule allI impI)+, goal_cases)
  ...
  def AX\<^sub>3 \<equiv> "num_of_big_int (word32_to_int \<circ> lsc__ec__point_add__x3) 0 ?L"
  def AY\<^sub>3 \<equiv> "num_of_big_int (word32_to_int \<circ> lsc__ec__point_add__y3) 0 ?L"
  def AZ\<^sub>3 \<equiv> "num_of_big_int (word32_to_int \<circ> lsc__ec__point_add__z3) 0 ?L"
  ...
  note bit1 = `(if (if elts e1 o1 AND _ =
  ...
  have ge0: "0 \<le> A" "0 \<le> INV" "0 \<le> X\<^sub>3\<^sub>2" "0 \<le> Y\<^sub>3\<^sub>2" "0 \<le> Z\<^sub>3\<^sub>2" "0 \<le> X\<^sub>3\<^sub>3" "0 \<le> Y\<^sub>3\<^sub>3" "0 \<le> Z\<^sub>3\<^sub>3"
    "0 \<le> DX\<^sub>2" "0 \<le> DY\<^sub>2" "0 \<le> DZ\<^sub>2" "0 \<le> AX\<^sub>3" "0 \<le> AY\<^sub>3" "0 \<le> AZ\<^sub>3"
  ...
  from ge0
    `(num_of_big_int' (Array lsc__ec__point_add__x3 _) _ _ < _) = _`
    `(num_of_big_int' (Array lsc__ec__point_add__y3 _) _ _ < _) = _`
    `(num_of_big_int' (Array lsc__ec__point_add__z3 _) _ _ < _) = _`
  have a\<^sub>3: "in_carrierp (AX\<^sub>3, AY\<^sub>3, AZ\<^sub>3)"
    by (simp add: in_carrierp_def res_carrier_eq defs)
  ...
  note two_add_eq =
    `point_add_spec _ _ _ _ _ _ _ _ (num_of_big_int' (Array lsc__ec__point_add__x3 _) _ _) _ _ = _`
      [my_simplified point_add_spec_def Let_def defs
       make_affine_proj_eq_iff a b on_curvep1 on_curvep2 a\<^sub>3 padd_in_carrierp padd_correct [of _ b]
       on_curvep_imp_in_carrierp [of ?a b], symmetric]

  with a b on_curvep1 on_curvep2
  have a\<^sub>3': "on_curvep ?a b (AX\<^sub>3, AY\<^sub>3, AZ\<^sub>3)"
    by (simp add: on_curvep_iff_on_curve [OF a b a\<^sub>3] add_closed
      on_curvep_iff_on_curve [symmetric])
  ...
  note eq3 =
    `point_add_spec _ _ _ _ _ _ _ _ (num_of_big_int' (Array x33 _) _ _) _ _ = _`
      [my_simplified point_add_spec_def Let_def slide_eq mk_bounds_eqs integer_in_range_def defs
       make_affine_proj_eq_iff a b p\<^sub>3\<^sub>3 d\<^sub>2' a\<^sub>3' padd_in_carrierp padd_correct [of _ b]
       on_curvep_imp_in_carrierp [of ?a b], symmetric]

  with a b d\<^sub>2' a\<^sub>3'
  ...
  from eq1 eq2 eq3 two_add_eq a b nonsingular on_curvep1 on_curvep2
  ...
  also from a b nonsingular on_curvep1 on_curvep2 have "\<dots> =
    ...
    by (simp add: nat_add_distrib num_of_lint_lower word32_to_int_lower
      pos_imp_zdiv_nonneg_iff on_curvep_iff_on_curve [symmetric] add_assoc [symmetric]
  ...
  have "(?e2 * 2 ^ nat (31 - j) +
      \<lfloor>elts e2 (e2_first + (o1 - e1_first))\<rfloor>\<^sub>s div 2 ^ nat (j + 1)) * 2 + 1 =
    ?e2 * 2 ^ nat (31 - j + 1) +
    \<lfloor>elts e2 (e2_first + (o1 - e1_first))\<rfloor>\<^sub>s div 2 ^ nat j div 2 * 2 + 1"
    by (simp only: nat_add_distrib)
      (simp add: zdiv_zmult2_eq [of 2, simplified mult.commute [of _ 2]])
  also from bit2
    power_increasing [OF nat_mono [OF `j \<le> 31`], of "2::int"] `0 \<le> j` `j \<le> 31`
