theory lscmnec_Lsc__ec__point_mult__subprogram_def_WP_parameter_def_10
imports "../Elliptic_Spec"
begin

why3_open "lscmnec_Lsc__ec__point_mult__subprogram_def_WP_parameter_def_10.xml"
...
proof (simp add: point_mult_spec_def Let_def, (rule allI impI)+, goal_cases)
  ...
  note bit = `(if (if elts e o1 AND _ ^ nat j = _ then _ else _) \<noteq> _ then _ else _) \<noteq> _`
  ...
  from
    `\<forall>k. _ \<longrightarrow> (_ \<longrightarrow> x23 k = _) \<and> _`
    `\<forall>k. _ \<longrightarrow> (_ \<longrightarrow> y23 k = _) \<and> _`
    `\<forall>k. _ \<longrightarrow> (_ \<longrightarrow> z23 k = _) \<and> _`
    `\<lfloor>x2__first\<rfloor>\<^sub>\<int> \<le> x2_first`
    `x2_first + (x1_last - x1_first) \<le> \<lfloor>x2__last\<rfloor>\<^sub>\<int>`
    `\<lfloor>y2__first\<rfloor>\<^sub>\<int> \<le> y2_first`
    `y2_first + (x1_last - x1_first) \<le> \<lfloor>y2__last\<rfloor>\<^sub>\<int>`
    `\<lfloor>z2__first\<rfloor>\<^sub>\<int> \<le> z2_first`
    `z2_first + (x1_last - x1_first) \<le> \<lfloor>z2__last\<rfloor>\<^sub>\<int>`
  have eq3: "(X\<^sub>2\<^sub>3, Y\<^sub>2\<^sub>3, Z\<^sub>2\<^sub>3) = (DX\<^sub>2, DY\<^sub>2, DZ\<^sub>2)"
    by (simp add: X\<^sub>2\<^sub>3_def Y\<^sub>2\<^sub>3_def Z\<^sub>2\<^sub>3_def DX\<^sub>2_def DY\<^sub>2_def DZ\<^sub>2_def num_of_lint_def
      slide_eq mk_bounds_eqs integer_in_range_def)
  
  from eq1 eq2 a b nonsingular on_curvep
  have "make_affine (DX\<^sub>2, DY\<^sub>2, DZ\<^sub>2) =
    point_mult ?a (nat ((?e * 2 ^ nat (31 - j) +
      \<lfloor>elts e o1\<rfloor>\<^sub>s div 2 ^ nat (j + 1)) * 2)) ?x"
  ...
  have "\<dots> = ?e * 2 ^ nat (31 - j + 1) +
    ...
    by (simp add: defs make_affine_proj_eq_iff on_curvep eq3 d\<^sub>2' a b
