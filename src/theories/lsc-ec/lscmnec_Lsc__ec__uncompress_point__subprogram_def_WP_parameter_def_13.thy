...
proof -
  ...
  moreover have "num_of_big_int (word32_to_int \<circ> y1) y_first ?l < Base ^ nat ?l"
    by (simp add: num_of_lint_upper word32_to_int_upper')
  ultimately show ?thesis using
    `y = y1` `l = x_last - x_first`
    `(_ = num_of_big_int' (Array y1 _) _ _ - _) = _`
    `result1 = slide lsc__bignum__mont_exp__a _ _`
    `is_zero (Array result1 _) _ _ \<noteq> True`
    `(is_zero (Array result1 _) _ _ = True) = _`
