theory lscmnec_Lsc__ec__point_add__subprogram_def_WP_parameter_def_31
imports "../Elliptic_Spec" "../Point_Add"
begin

why3_open "lscmnec_Lsc__ec__point_add__subprogram_def_WP_parameter_def_31.xml"
...
proof -
  ...
  moreover from
    ...
    `is_zero (Array (slide lsc__bignum__mod_sub__a1 _ _) _) _ _ = _`
  have "(Y\<^sub>2 * Z\<^sub>1 - Y\<^sub>1 * Z\<^sub>2) * INV mod M = 0"
    by (simp add: mk_bounds_eqs integer_in_range_def slide_eq defs defs' mod_sub_eq ring_distribs)
  then have "(Y\<^sub>2 * Z\<^sub>1 - Y\<^sub>1 * Z\<^sub>2) mod M = 0"
    by (simp add: INV_def base_eq eq0_inv_iff' [OF Base_inv])
