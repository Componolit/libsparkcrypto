...
proof -
  ...
  from
    ...
    `\<forall>k. \<lfloor>x2_first\<rfloor>\<^sub>\<int> \<le> k \<and> k \<le> \<lfloor>x2_last\<rfloor>\<^sub>\<int> \<longrightarrow> _`
    `\<lfloor>x2_first\<rfloor>\<^sub>\<int> \<le> \<lfloor>x2_first1\<rfloor>\<^sub>\<nat>`
    `\<lfloor>x2_first1\<rfloor>\<^sub>\<nat> + (\<lfloor>x1_last\<rfloor>\<^sub>\<nat> - \<lfloor>x1_first\<rfloor>\<^sub>\<nat>) \<le> \<lfloor>x2_last\<rfloor>\<^sub>\<int>`
  have "num_of_big_int (word32_to_int \<circ> x23) \<lfloor>x2_first1\<rfloor>\<^sub>\<nat> ?l =
