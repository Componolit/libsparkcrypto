theory lscmnsha512_Lsc__sha512__hash_context__subprogram_def_WP_parameter_def_3
imports "../SPARK2014"
begin

why3_open "lscmnsha512_Lsc__sha512__hash_context__subprogram_def_WP_parameter_def_3.xml"
...
proof -
  ...
  from `length1 mod of_int 1024 \<noteq> of_int 0`
    uint_lt [of length1] uint_0 [of length1]
    ...
    `BV64.ule (unsigned_64_to_rep (first (rt message)))
       (unsigned_64_to_rep (last (rt message)))`
  have length:
    "uint (length1 div 1024) \<le> uint (last (rt message)) - uint (first (rt message))"
